...
export default function GerarPgrsPinhaisPage() {
  ...
  useEffect(() => {
    async function load() {
      ...
      setForm((prev) => ({
        ...
        responsavelAssinaturaNome: empresa.dirigenteNome || "",
        responsavelAssinaturaCargo: empresa.dirigenteCargo || "",
      }));
      setDadosEstab({
  ...
  async function handleGerar() {
    ...
    try {
      const res = await fetch("/api/documentos/gerar", {
        ...
        body: JSON.stringify({ empresaId, templateSlug: "pgrs-pinhais", formData: form, dadosEstabelecimento: dadosEstab }),
      });
      if (!res.ok) {
      ...
      a.href = url;
      a.download = `PGRS_Pinhais_${empresaNome.replace(/[^a-zA-Z0-9]/g, "_")}.docx`;
      a.click();
  ...
  return (
    <div className="space-y-8">
      <Topbar title="PGRS Simplificado — Pinhais" subtitle={empresaNome} />
