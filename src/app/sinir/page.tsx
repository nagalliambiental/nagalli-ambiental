...
interface ContatoEmpreendimento {
  ...
  ativo: boolean;
  empreendimento: { id: number; apelido: string; unidadeSinir: string | null; cliente: { apelido: string } };
}
...
export default function SinirPage() {
  ...
  return (
    <div>
      ...
      {tab === "meusMtrs" && (
        <MeusMtrsTab conexoes={conexoes} empreendimentos={empreendimentos} filtro={filtro} setFiltro={setFiltro} onVerificar={() => carregarManifestos()} toast={toast} />
      )}
...
function MeusMtrsTab(props: {
  ...
  const { conexoes, empreendimentos, filtro, setFiltro, onVerificar, toast } = props;
  const [conexaoId, setConexaoId] = useState("");
  ...
  const salvosParaNotificar = lista.filter((m) => m.status === "SALVO");
  const conexaoAtual = conexoes.find((c) => c.id === Number(conexaoEfetiva));
  const empreendimentoAtual = empreendimentos.find((e) => e.id === conexaoAtual?.empreendimentoId);
  ...
  function alternarContato(id: number) {
    setSelecionados((atual) => (atual.includes(id) ? atual.filter((x) => x !== id) : [...atual, id]));
  }

  async function enviarNotificacao() {
    if (selecionados.length === 0) {
      toast("Selecione pelo menos um contato", "error");
      return;
    }
    setEnviandoNotif(true);
    try {
      const res = await fetch("/api/sinir/notificar-salvos", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ conexaoId: Number(conexaoEfetiva), contatoIds: selecionados }),
      });
      const data = await res.json().catch(() => null);
      if (!res.ok) {
    ...
    <div className="space-y-4">
      ...
      {modalNotificar && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
          <div className="shadow-card w-full max-w-lg rounded-[var(--radius-card)] border border-[var(--color-paper-200)] bg-white">
            <div className="flex items-center justify-between border-b border-[var(--color-paper-200)] px-5 py-4">
              <h3 className="font-display flex items-center gap-2 text-base font-semibold text-[var(--color-ink-900)]">
                <Mail size={17} className="text-[var(--color-brand-600)]" />
                Notificar MTRs salvos por e-mail
              </h3>
              <button onClick={() => setModalNotificar(false)} className="rounded-md p-1 text-[var(--color-ink-500)] hover:bg-[var(--color-paper-100)]">
                <X size={16} />
              </button>
            </div>
            ...
            <div className="px-5 py-4">
              ...
              {carregandoContatos ? (
                ...
                <div className="max-h-64 space-y-1 overflow-y-auto">
                  {contatosNotif.map((c) => (
                    <label key={c.id} className="flex cursor-pointer items-center gap-2 rounded-lg px-2 py-1.5 text-sm hover:bg-[var(--color-paper-50)]">
                      <input
                        type="checkbox"
                        checked={selecionados.includes(c.id)}
                        onChange={() => alternarContato(c.id)}
                        className="accent-[var(--color-brand-600)]"
                      />
                      <span className="font-medium text-[var(--color-ink-900)]">{c.nome}</span>
                      {c.cargo && <span className="text-xs text-[var(--color-ink-500)]">({c.cargo})</span>}
                      <span className="ml-auto text-xs text-[var(--color-ink-500)]">{c.email}</span>
                    </label>
                  ))}
                </div>
              )}
            ...
            <div className="flex justify-end gap-2 border-t border-[var(--color-paper-200)] px-5 py-3">
              <button onClick={() => setModalNotificar(false)} className="rounded-lg border border-[var(--color-paper-200)] px-4 py-2 text-sm font-medium text-[var(--color-ink-700)] hover:bg-[var(--color-paper-50)]">
                Voltar
              </button>
              <button
                onClick={enviarNotificacao}
                disabled={enviandoNotif || selecionados.length === 0}
                className="focus-ring transition-brand flex items-center gap-2 rounded-lg bg-[var(--color-brand-500)] px-4 py-2 text-sm font-medium text-white hover:bg-[var(--color-brand-600)] disabled:opacity-50"
              >
                {enviandoNotif ? <Loader2 size={15} className="animate-spin" /> : <Send size={15} />}
                {enviandoNotif ? "Enviando..." : `Enviar para ${selecionados.length || 0} contato(s)`}
              </button>
            </div>
          </div>
        </div>
